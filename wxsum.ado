...
if "`ini_day'" == "" {
	loc ini_day = "`day_month'"
}
if "`fin_day'" == "" {
	loc fin_day = "`day_month'"
}
        ...
        foreach month of loc months {
            foreach day of loc days {
                ...
                * Determine if the current date is within the season bounds
                loc in_season = 0
                if `ini_month' <= `fin_month' {
                    if (`current_year' == `season_start_year' & ((`candidate_month' == `ini_month' & `candidate_day' >= `ini_day') | (`candidate_month' > `ini_month' & `candidate_month' < `fin_month') | (`candidate_month' == `fin_month' & `candidate_day' <= `fin_day'))) {
                        loc in_season = 1
                    }
                }
                else {
                    if (`current_year' == `season_start_year' & ((`candidate_month' == `ini_month' & `candidate_day' >= `ini_day') | (`candidate_month' > `ini_month'))) | (`current_year' == `season_end_year' & ((`candidate_month' < `fin_month') | (`candidate_month' == `fin_month' & `candidate_day' <= `fin_day'))) {
                        loc in_season = 1
                    }
                }
...
if "`prefix'" != "" {
	if "`rain_data'" == "rain_data" {
		foreach v of varlist *season* *norain* *raindays* dry* *percent_raindays* {
			capture rename `v' `prefix'`v'
		}
	}
	if "`temperature_data'" == "temperature_data" {
		foreach v of varlist *season* *gdd* tempbin* {
			capture rename `v' `prefix'`v'
		}
	}
}
...
if "`keep'" != "" {
	if "`rain_data'" == "rain_data" {
		...
		if "`prefix'" != "" {
			qui: keep `keep' *season* *norain* *raindays* `prefix'dry* *percent_raindays*
			qui: drop `prefix'z_raindays* `prefix'z_norain* `prefix'max* `prefix'z_percent*
		}
		else {
	...
	if "`temperature_data'" == "temperature_data" {
		if "`prefix'" != "" {
			qui: keep `keep' *season* *gdd* `prefix'tempbin*
			qui: drop `prefix'total_season_*
		}
		else {
