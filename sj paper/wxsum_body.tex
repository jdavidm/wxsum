...
The deviation and z-score variables (\texttt{dev\_} and \texttt{z\_} prefixes) are computed with a rolling window of \texttt{lr\_years} strictly preceding seasons. Let $w_{it}$ denote a seasonal weather statistic for location $i$ in the season beginning in year $t$, and let $L$ denote the number of prior years specified in \texttt{lr\_years()}. The rolling long-run mean and standard deviation are
\[
\bar{w}_{it}^{L}=L^{-1}\sum_{l=1}^{L} w_{i,t-l}
\qquad\text{and}\qquad
s_{it}^{L}=\left\{(L-1)^{-1}\sum_{l=1}^{L}
        \left(w_{i,t-l}-\bar{w}_{it}^{L}\right)^2\right\}^{1/2}
\]
The deviation is $w_{it}-\bar{w}_{it}^{L}$, and the z-score is $(w_{it}-\bar{w}_{it}^{L})/s_{it}^{L}$. The use of strictly prior years avoids including the current realization in its own benchmark. If fewer than $L$ preceding seasons are available, deviations and z-scores are not generated for those early seasons.
