...
The \texttt{wxsum} command allows the user to customize season dates, GDD bounds, KDD thresholds, GDD category widths and endpoints, fixed daily-temperature bin counts and endpoints, the rainfall threshold defining a rainy day, the length of the long-run reference window, and the final output shape. This flexibility is important because the relevant exposure period, temperature sensitivity, and baseline climate vary across crops, agroecological zones, and research questions. For example, precipitation may matter most during planting and flowering stages, while heat exposure becomes critical during grain filling. The command accommodates these differences without requiring the user to reshape or pre-process the data beyond the standard wide format.

The choice of metric should follow from the research question and the mechanism under study. For applications focused on crop yields or agricultural output, growing degree days and killing degree days capture the accumulated heat exposure relevant to plant development and damage. GDD categories provide a less-parametric approach in which the seasonal GDD total is classified into fixed-width intervals, following the degree-day specification of \citet{DescheneGreenstone07}. Total seasonal rainfall and rainy-day counts are natural measures when precipitation levels directly drive outcomes, as in studies of agricultural productivity or water availability. Z-scores and deviations from long-run averages are suited to identifying weather shocks, seasons that are unusual relative to local climate, and are common in studies of consumption, poverty, and conflict. Fixed daily-temperature bin counts summarize the seasonal distribution of observed daily temperatures across common absolute temperature intervals. When only one daily temperature reading is available per day, these counts approximate the fixed-bin intuition used in work on nonlinear temperature--outcome relationships, such as that of Schlenker and Roberts (2009), though they do not reproduce true within-day exposure bins. Dry spells capture the timing and persistence of rainfall deficits, which may matter more than total rainfall for rain-fed agriculture. These are not exclusive categories; many applications combine several metrics to capture different dimensions of weather exposure.
...
\begin{stsyntax}
	...
	type({\it rain|temp\/}) [ ini\_day({\it day\/}) fin\_day({\it day\/})
    ...
    tmp\_binhi({\it num\/}) shape({\it wide|long\/})
    lr\_years({\it num\/}) keep({\it varlist\/}) save({\it filename\/}) ]
\end{stsyntax}

\noindent Here \textit{prefix} is the common stub for the variables. It is not a varlist. If the data contain variables named \texttt{rf\_19790101}, \texttt{rf\_19790102}, and so on, the prefix supplied to the command is \texttt{rf\_}. The \texttt{type()} option must be specified as either \texttt{rain} or \texttt{temp}.
...
\begin{table}[htbp]
	...
	\begin{center}
		\begin{tabularx}{\textwidth}{lX}
			...
			Rainfall data & Numeric daily precipitation variables; use \texttt{type(rain)}. \\
			Temperature data & Numeric daily temperature variables; use \texttt{type(temp)}. \\
...
\begin{description}
	...
	\item[\texttt{fin\_day(day)}] specifies the last day of the final month to be included in the season. If not specified, it dynamically defaults to the true last calendar day of the final month (handling leap years correctly).
	...
	\item[\texttt{gdd\_bin(num)}] specifies the fixed width of seasonal GDD categories. When specified, the command creates an integer categorical variable \texttt{gddcat\_}\textit{YYYY} for each generated GDD season. The argument is a positive numeric bin width, expressed in the units of the generated GDD variable. Valid only with \texttt{type(temp)}.
	\item[\texttt{gdd\_binlo(num)}] specifies the optional lower endpoint for regular fixed-width GDD categories. The default is 0 when \texttt{gdd\_bin()} is specified. If any seasonal GDD total falls below this value, a bottom-coded category is created. Requires \texttt{gdd\_bin()}.
	\item[\texttt{gdd\_binhi(num)}] specifies the optional upper endpoint for regular fixed-width GDD categories. Values at or above this endpoint are assigned to a top-coded category. When omitted, the command determines an upper endpoint from the pooled empirical maximum of generated seasonal GDD values and the chosen bin width. Requires \texttt{gdd\_bin()}. Must be greater than \texttt{gdd\_binlo()}. The range between \texttt{gdd\_binhi()} and \texttt{gdd\_binlo()} must be evenly divisible by \texttt{gdd\_bin()}.
	\item[\texttt{tmp\_bin(num)}] specifies the total number of fixed daily-temperature bin count variables to create per season. Must be a positive integer from 1 through 42. Valid only with \texttt{type(temp)}. Requires \texttt{tmp\_binlo()} and \texttt{tmp\_binhi()}. Creates variables \texttt{tmpbin01\_}\textit{YYYY} through \texttt{tmpbin}\textit{JJ}\texttt{\_}\textit{YYYY}.
	\item[\texttt{tmp\_binlo(num)}] specifies the lower endpoint for fixed daily-temperature bin counts. Required when \texttt{tmp\_bin()} is specified. Must be in the same units as the daily temperature data. No default.
	\item[\texttt{tmp\_binhi(num)}] specifies the upper endpoint for fixed daily-temperature bin counts. Required when \texttt{tmp\_bin()} is specified. Must be greater than \texttt{tmp\_binlo()}. No default.
	...
	\item[\texttt{keep(varlist)}] retains variables from the input dataset in the output dataset. This option is typically used for identifiers, administrative codes, coordinates, or other merge keys. When \texttt{shape(long)} is specified, users are strongly encouraged to include unit identifiers in \texttt{keep()} so the long output contains an explicit merge key.
...
When \texttt{shape(long)} is specified, the output is stacked with one row per retained unit-year. A variable named \texttt{year} identifies the season-start year. Generated variables are renamed without their year suffixes; for example, \texttt{mean\_1993} becomes \texttt{mean} with \texttt{year == 1993}, \texttt{gddcat\_1993} becomes \texttt{gddcat}, and \texttt{tmpbin01\_1993} becomes \texttt{tmpbin01}. Variables specified in \texttt{keep()} are repeated across years. Users are strongly encouraged to include unit identifiers in \texttt{keep()} when using \texttt{shape(long)}, because otherwise the long output may not contain an explicit merge key. The \texttt{shape(long)} option does not change the requirement that input data be wide; it is a final-output stacking step only.
...
\begin{table}[htbp]
	...
	\begin{center}
		\begin{tabularx}{\textwidth}{lX}
			...
			GDD categories & Integer category variable \texttt{gddcat\_}\textit{YYYY} identifying fixed-width intervals of the seasonal GDD total, with Stata value labels defining the intervals. Created when \texttt{gdd\_bin()} is specified. \\
			Daily temperature bins & Count variables \texttt{tmpbin01\_}\textit{YYYY} through \texttt{tmpbin}\textit{JJ}\texttt{\_}\textit{YYYY} recording the number of observed daily temperature readings falling in each fixed absolute temperature interval. Created when \texttt{tmp\_bin()} is specified. \\
			\hline
		\end{tabularx}
	\end{center}
\end{table}
...
The skewness variable is the raw third standardized moment,

\begin{equation}
	\texttt{skew\_} t_i = \frac{1}{N_{it}} \sum_{\substack{d \in \mathcal{S}_{t} \\ X_{id} \neq \text{missing}}} \left( \frac{X_{id} - \texttt{mean\_} t_i}{\texttt{sd\_} t_i} \right)^3 .
\end{equation}

\noindent The raw third moment variable requires at least 3 non-missing observations. When $N_{it} < 3$ or the standard deviation is zero, the skewness variable is missing.
...
\noindent The variable \texttt{mon\_}$t$ is the mean of the monthly totals $M_{imt}$ across the months represented in the season.
...
The variable \texttt{dry\_}$t$ is the longest mid-season dry spell: the maximum run of consecutive non-missing days with $R_{id} < c$ that occurs strictly after the first observed rainy day and strictly before the last observed rainy day in the season. Leading dry days before the first rainy day and trailing dry days after the last rainy day are excluded from \texttt{dry\_}$t$.

The variable \texttt{dry\_start\_}$t$ is the number of consecutive non-missing dry days at the start of the season before the first rainy day or missing value.

The variable \texttt{dry\_end\_}$t$ is the number of consecutive non-missing dry days at the end of the season after the last rainy day or missing value, counted backward from the final day of the season.

If all daily rainfall values in a location-season are missing, all three dry-spell variables are missing. Missing rainfall values break dry spells rather than extending them.
...
\subsubsection{GDD categories}

GDD categories provide a less-parametric degree-day specification in the spirit of \citet{DescheneGreenstone07}. These categories classify the seasonal GDD total, not daily temperatures, into fixed-width intervals. When \texttt{gdd\_bin()} is specified, let $\text{GDD}_{it}$ denote seasonal growing degree-days computed as above, and let $\tau_0 < \tau_1 < \cdots < \tau_J$ denote fixed GDD cutpoints generated from \texttt{gdd\_bin()}, \texttt{gdd\_binlo()}, and either \texttt{gdd\_binhi()} or the automatically determined upper endpoint. For regular categories:

\begin{equation}
	\texttt{gddcat}_{it} = j \quad \text{if} \quad \tau_{j-1} \leq \text{GDD}_{it} < \tau_j.
\end{equation}

\noindent If a bottom-coded category is needed because some seasonal GDD totals fall below \texttt{gdd\_binlo()}:

\begin{equation}
	\texttt{gddcat}_{it} = 1 \quad \text{if} \quad \text{GDD}_{it} < \tau_0.
\end{equation}

\noindent If a top-coded category is requested via \texttt{gdd\_binhi()}:

\begin{equation}
	\texttt{gddcat}_{it} = J + 1 \quad \text{if} \quad \text{GDD}_{it} \geq \tau_J.
\end{equation}

\noindent The variable \texttt{gddcat\_}\textit{YYYY} is an integer-valued categorical variable. Stata value labels define the GDD intervals, so that \texttt{tabulate gddcat\_}\textit{YYYY} displays the interval boundaries. Users can employ factor-variable notation, such as \texttt{i.gddcat\_}\textit{YYYY} in wide output or \texttt{i.gddcat} after \texttt{shape(long)}, to create regression indicators. These categories are defined over the seasonal GDD total, not over daily temperatures. The command is unit agnostic: \texttt{gdd\_bin()}, \texttt{gdd\_binlo()}, and \texttt{gdd\_binhi()} must be expressed in the units of the generated GDD variable.

\subsubsection{Fixed daily-temperature bin counts}

Fixed daily-temperature bin counts summarize the seasonal distribution of observed daily temperatures across common absolute temperature intervals. These variables count the number of nonmissing daily temperature readings falling in each interval and are created when \texttt{tmp\_bin()} is specified.
...
Missing daily temperatures are not counted in any bin. If all daily temperatures for a location-season are missing, all bin variables for that location-season are set to missing. Otherwise, the sum of all bin counts equals the number of nonmissing daily temperature readings in the season.

These are fixed daily-temperature bin counts using one observed daily reading per day. They approximate the fixed-bin intuition used in work on nonlinear temperature--outcome relationships, such as that of Schlenker and Roberts (2009), but they do not reproduce true within-day exposure bins because \texttt{wxsum} does not allocate fractions of each day across temperature intervals from minimum/maximum or subdaily data.
...
GDD category variables (\texttt{gddcat\_}\textit{YYYY}) and daily-temperature bin count variables (\texttt{tmpbin}\textit{JJ}\texttt{\_}\textit{YYYY}) are not used to compute long-run deviations or $z$-scores. The long-run reference calculations apply only to the original supported shock measures: seasonal totals, rainy-day counts, rain-day shares, GDD, and KDD.
...
\begin{stlog}
	...
	>     gdd_lo(8) gdd_hi(32) kdd_base(32) ///
	>     keep(id region)
\end{stlog}
...
\subsection{GDD categories}

To create Desch{\^e}nes--Greenstone-style GDD categories, the user specifies \texttt{gdd\_bin()} with a bin width in the units of the generated GDD variable:

\begin{stlog}
	. use temp.dta, clear
	. wxsum tmp_, ini_month(04) ini_day(01) fin_month(09) ///
	>     fin_day(30) temp_data gdd_lo(46.4) gdd_hi(89.6) ///
	>     gdd_bin(500) keep(id region)
\end{stlog}

\noindent Here \texttt{gdd\_bin(500)} means 500 degree-days in the units of the generated GDD variable. This example assumes Fahrenheit temperatures and Fahrenheit degree-days. If the temperature data are measured in Celsius, the user must supply Celsius thresholds and a Celsius-degree-day bin width. The command does not infer or convert units.

\subsection{Fixed daily-temperature bin counts}

To create daily-temperature bin counts, the user specifies \texttt{tmp\_bin()} along with \texttt{tmp\_binlo()} and \texttt{tmp\_binhi()}:

\begin{stlog}
	. use temp.dta, clear
	. wxsum tmp_, ini_month(04) ini_day(01) fin_month(09) ///
	>     fin_day(30) temp_data gdd_lo(8) gdd_hi(32) ///
	>     tmp_bin(15) tmp_binlo(0) tmp_binhi(39) ///
	>     keep(id region)
\end{stlog}
...
\begin{stlog}
	...
	>     fin_day(30) temp_data gdd_lo(8) gdd_hi(32) ///
	>     tmp_bin(15) tmp_binlo(0) tmp_binhi(39) ///
	>     keep(id region) shape(long)
\end{stlog}

\noindent The output has one row per \texttt{id}-year. The time variable is named \texttt{year}. Variables are named \texttt{gdd}, \texttt{tmpbin01}, \texttt{tmpbin02}, and so on, rather than \texttt{gdd\_}\textit{YYYY}, \texttt{tmpbin01\_}\textit{YYYY}, etc.
...
\noindent Changing \texttt{lr\_years()} affects only the rolling deviations and $z$ scores. It does not affect the seasonal means, medians, standard deviations, skewness measures, totals, counts, shares, dry-spell measures, GDD categories, or daily-temperature bin counts.

\section{Conclusions}

\texttt{wxsum} reduces the amount of custom code needed to transform daily weather data into seasonal measures commonly used in socioeconomic analysis. It standardizes the construction of summary statistics, threshold-based measures, rainy-day counts, dry spells, GDD categories, fixed daily-temperature bin counts, and rolling weather-shock measures, while leaving substantive choices, like data source, spatial match, season definition, units, and threshold definitions, with the researcher. An implementation for R has also been developed, extending the same core functionality to users who work outside Stata or who integrate weather processing into R-based data pipelines. This division of labor is useful: the command reduces the likelihood of programming errors in repetitive weather-processing tasks, but it does not substitute for careful research design.

Users can customize the season definition, the thresholds for growing and killing degree days, GDD category widths and endpoints, fixed daily-temperature bin counts and endpoints, the rainfall threshold, and the length of the long-run window used to compute deviations and z-scores. The command handles cross-year seasons, missing data, and large datasets without requiring users to reshape or preprocess their data beyond the standard wide format. Users can also choose between wide and long output formats with the \texttt{shape()} option.

%Future development could extend \texttt{wxsum} in several directions. First, diagnostic options that report missing days, season length, and the number of prior years available for each generated shock measure would make quality control easier in large applications. Second, the command could be expanded to additional weather and climate variables, such as humidity, wind speed, vapor pressure deficit, or solar radiation, when these variables are available in daily gridded products. These extensions would preserve the main purpose of the command: to make weather-data processing in Stata more transparent, reproducible, and less error prone.
