...
\begin{description}
	...
	\item[\texttt{tmp\_bin(num)}] specifies the total number of fixed daily-temperature bin count variables to create per season. Must be a positive integer from 1 through 42. Valid only with \texttt{temp\_data}. Requires \texttt{tmp\_binlo()} and \texttt{tmp\_binhi()}. Creates variables \texttt{tmpbin01\_}\textit{YYYY} through \texttt{tmpbin}\textit{JJ}\texttt{\_}\textit{YYYY}.
